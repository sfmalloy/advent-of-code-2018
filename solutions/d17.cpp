...
i64 fill(std::unordered_map<Vec2, i32>& map, Vec2 pos, i64 max_r) {
    if (pos.r > max_r) {
        return VOID;
    }
    if (!map.contains(pos)) {
    ...
    i64 below = fill(map, Vec2(pos.r + 1, pos.c), max_r);

    if (below == WALL or below == RESTING_WATER) {
        bool left_resting = true;
        i64 left = pos.c - 1;
        for (; map[Vec2(pos.r, left)] != WALL; --left) {
            i64 down = fill(map, Vec2(pos.r + 1, left), max_r);
        ...
        i64 right = pos.c + 1;
        for (; map[Vec2(pos.r, right)] != WALL; ++right) {
            i64 down = fill(map, Vec2(pos.r + 1, right), max_r);
            if (down != WALL and down != RESTING_WATER) {
                right_resting = false;
                ++right;
                break;
        ...
        for (i64 c = left + 1; c < right; ++c) {
            map[Vec2(pos.r, c)] = resting ? RESTING_WATER : FLOWING_WATER;
        }
    }
    return map[pos];
}
...
{
    std::unordered_map<Vec2, i32> map;
    std::string line;
